...
export class NextAppointmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.initialiseForms()
      ...
      if(expertId)
      this.expertService.upcomingAppointment({expertId:expertId}).subscribe({
        next:(Response)=>{
...
initialiseForms():void{
  ...
  enterRoom(){
    this.expertService.share_roomId_through_email({roomId:this.roomId,slotId:this.upcoming_appointment._id}).subscribe({
      next:(Response)=>{
    ...
    if(this.roomId){
      this.router.navigate(['/expert/expert_video_call_room',this.roomId,this.upcoming_appointment._id])
    }else{
