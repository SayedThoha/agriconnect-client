...
@Component({
  ...
  imports: [CommonModule,ReactiveFormsModule,FormsModule,RouterModule,TableModule,ScrollPanelModule,CalendarModule],
...
export class SlotAddingComponent implements OnInit {
  ...
  addSlots(time: string, index: number) {
    if (!this.date) {
      ...
      const data = { time: time, _id: this.expertId }; //time example: 2024-08-11T13:00:00.000+00:00
  ...
  defaultSlots(){
    if(this.slots_for_display.length===0){
      ...
      this.expertService.add_all_slots({slots:this.slots_for_display,expertId:this.expertId}).subscribe({
        next:(Response)=>{
